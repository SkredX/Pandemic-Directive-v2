...
def calculate_score(stats, ending_type):
    # Base Score: Days survived + Population Health + Trust
...
RANDOM_POOL = {
    ...
    "e13": { "text": "SITUATION: WATER CONTAMINATION\nA filtration cycle was missed. The water is safe but looks cloudy.", "choices": [{"text": "Fix immediately (Shutdown).", "mods": {"eco": -8}}, {"text": "Ration water usage.", "mods": {"trust": -10}}, {"text": "Cover it up.", "mods": {"pop": -5, "trust": -20}}] },
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        try:
            ...
            # ROUTE: FETCH LEADERBOARD
            if data.get('action') == 'get_leaderboard':
                if leaderboard_col:
                    top_scores = list(leaderboard_col.find({}, {'_id': 0}).sort("score", DESCENDING).limit(10))
                    self.send_json({"leaderboard": top_scores})
                else:
            ...
            if data.get('action') == 'submit_score':
                ...
                if leaderboard_col and user_id and stats:
                    final_score = calculate_score(stats, ending)
                    # UPSERT SCORE
                    existing = leaderboard_col.find_one({"user_id": user_id})
                    if not existing or final_score > existing['score']:
                        leaderboard_col.update_one(
                            {"user_id": user_id},
                            {"$set": {"name": name, "score": final_score, "days": stats['day'], "ending": ending, "date": stats['day']}},
                            upsert=True
                        )
                    self.send_json({"status": "saved", "score": final_score})
                return

            # ROUTE: GAME TURN (STANDARD)
            stats = data.get('stats', {})
            ...
            if not is_init and global_choices and last_event_id and choice_idx is not None:
                try:
                    key = f"{last_event_id}_{choice_idx}"; total_key = f"{last_event_id}_total"
                    global_choices.update_one({"_id": key}, {"$inc": {"count": 1}}, upsert=True)
                    global_choices.update_one({"_id": total_key}, {"$inc": {"count": 1}}, upsert=True)
                    c_doc = global_choices.find_one({"_id": key}); t_doc = global_choices.find_one({"_id": total_key})
                    if c_doc and t_doc: pct = int((c_doc['count'] / t_doc['count']) * 100); global_msg = f"\n[GLOBAL INTEL]: {pct}% of commanders chose this."
                except: pass
